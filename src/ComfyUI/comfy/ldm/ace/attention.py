...
class CustomLiteLAProcessor2_0:
    ...
    def __call__(
        ...
        **kwargs,
        ...
        # RoPEéœ€è¦ [B, H, S, D] è¾“å…¥
        # æ­¤æ—¶ queryæ˜¯ [B, H, D, S], éœ€è¦è½¬æˆ [B, H, S, D] æ‰èƒ½åº”ç”¨RoPE
        query = query.permute(0, 1, 3, 2)  # [B, H, S, D]  (ä»Ž [B, H, D, S])
        ...
        # æ­¤æ—¶ queryæ˜¯ [B, H, S, D]ï¼Œéœ€è¦è¿˜åŽŸæˆ [B, H, D, S]
        ...
        if attention_mask is not None:
            ...
            if not attn.is_cross_attention:
                key = key * attention_mask  # key: [B, h, S, D] ä¸Ž mask [B, 1, S, 1] ç›¸ä¹˜
                value = value * attention_mask.permute(0, 1, 3, 2)  # å¦‚æžœ value æ˜¯ [B, h, D, S]ï¼Œé‚£ä¹ˆéœ€è°ƒæ•´maskä»¥åŒ¹é…Sç»´åº¦
        ...
        if attn.is_cross_attention and encoder_attention_mask is not None and has_encoder_hidden_state_proj:
            ...
            # æ­¤æ—¶ key: [B, h, S_enc, D], value: [B, h, D, S_enc]
    ...
    def __call__(
        ...
        **kwargs,
        ...
        if attn.is_cross_attention and encoder_attention_mask is not None and has_encoder_hidden_state_proj:
            ...
            # cross attention æ•´åˆattention_maskå’Œencoder_attention_mask
